...
class MultipleLinearRegressor:
    ...
    def fit(self, observations: np.ndarray, ground_truth: np.ndarray) ->  None:
        ...
        if self.__validate_observations_ground_truth(observations, ground_truth):
            ...
            #implement formula
            optimal_parameters = np.dot(np.dot((np.dot(trans_observations, observations_prime) ** (-1)), trans_observations), ground_truth)
    ...
    def __validate_observations_ground_truth(self, observations: np.ndarray, ground_truth: np.ndarray) -> bool:
